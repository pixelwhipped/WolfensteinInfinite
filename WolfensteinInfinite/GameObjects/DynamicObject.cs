...
namespace WolfensteinInfinite.GameObjects
{
    ...
    public class EnemyObject : DynamicObject
    {
        ...
        private const float MeleeAttackRange = 2.0f;
    ...
    : base(x, y, DynamicObjectType.Enemy, sprite)
        {
            ...
            // Diminishing returns scaling — levels off around level 50+
        ...
        private bool HasLineOfSight(InGameState state)
        {
            ...
            for (int i = 0; i < steps; i++)
            {
                ...
                if (tile >= 0) return false; // wall blocks LOS
                if (tile == InGameState.DOOR_TILE)
                {
                    // Closed door blocks LOS, open door does not
        ...
        private void TryAttack(float frameTime, InGameState state)
        {
            ...
            // Force animation reset on each new attack
            _lastAIState = EnemyAIState.Idle; // clear cache so SetAnimation fires
