namespace WolfensteinInfinite.WolfMod
{
    ...
    {
        public static (MapSection map, IEnumerable<(int X, int Y)> keys) ToMap(int xOffset, int yOffset, MapSection mapSection)
        {
            
            //var m = mapSection.Clone();
            return (mapSection, mapSection.GetConnections(xOffset,yOffset));            
        }

        public static (MapGeneratorSection map, IEnumerable<(int X, int Y)> keys) ToMap(int xOffset, int yOffset, MapGeneratorSection mapSection)
        {
            //var m = mapSection.Clone();
            return (mapSection, mapSection.Section.GetConnections(xOffset, yOffset));
        }
        ...
        {
            return HashCode.Combine(X, Y, Guid);
        }
